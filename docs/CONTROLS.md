...
| 좌1 | ▶ / ⏸ | 초록/빨강 | ▶ |
| 좌2 | {n}x | 흰색 | 5x |
| 좌3 | Y{n} M{n} D{n} {H}:00 | 흰색 | Y3 M7 D15 14:00 |
...
| 정보 | 직업 \| 정착지 \| 나이 | Gatherer \| S1 \| Age: 12.3y |
...
모든 팝업(통계/엔티티/건물 상세)은 PopupManager(CanvasLayer, layer=100)가 관리:
1. **배경 클릭**: dim_bg(반투명 검정) 클릭 시 닫힘. 패널 내부 클릭은 MOUSE_FILTER_STOP으로 차단되어 닫히지 않음
2. **키보드**: G(통계 토글)/E(디테일 토글)/Esc(닫기)
3. **도움말**: H 키 토글 (도움말은 별도, PopupManager 외부)
