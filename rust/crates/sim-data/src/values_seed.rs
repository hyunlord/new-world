...
pub fn hexaco_seed_map() -> &'static [(ValueType, &'static [(&'static str, f64)])] {
    &[
        ...
        (
            ValueType::Loyalty,
            &[
                ("E_sentimentality", 0.30),
                ("E_dependence", 0.25),
                ("H_sincerity", 0.25),
                ("A_forgivingness", 0.20),
            ],
        ),
        (
            ValueType::Family,
            &[
                ("E_sentimentality", 0.40),
                ("E_dependence", 0.25),
                ("H_sincerity", 0.20),
                ("A_patience", 0.15),
            ],
        ...
        (
            ValueType::Friendship,
            &[
                ("X_sociability", 0.30),
                ("E_sentimentality", 0.25),
                ("H_sincerity", 0.25),
                ("A_forgivingness", 0.20),
            ],
        ),
        (
            ValueType::Power,
            &[
                ("X_social_boldness", 0.30),
                ("H_modesty", -0.35),
                ("H_greed_avoidance", -0.25),
                ("X_social_self_esteem", 0.10),
            ],
        ),
        (
            ValueType::Truth,
        ...
        (
            ...
            &[
                ("H_sincerity", -0.30),
                ("O_unconventionality", 0.25),
                ("O_creativity", 0.25),
                ("X_social_boldness", 0.20),
            ],
        ),
        (
            ValueType::Eloquence,
        ...
        (
            ...
            &[
                ("O_aesthetic", 0.50),
                ("O_creativity", 0.30),
                ("O_unconventionality", 0.20),
            ],
        ),
        (
            ValueType::Cooperation,
            &[
                ("A_flexibility", 0.30),
                ("A_gentleness", 0.25),
                ("A_patience", 0.25),
        ...
        (
            ValueType::Introspection,
            &[
                ("O_inquisitiveness", 0.30),
                ("O_aesthetic", 0.25),
                ("O_creativity", 0.25),
                ("C_prudence", 0.20),
            ],
        ...
        (
            ValueType::SelfControl,
            &[
                ("C_prudence", 0.35),
                ("C_perfectionism", 0.25),
                ("C_organization", 0.20),
                ("A_patience", 0.20),
            ],
        ),
        (
            ValueType::Tranquility,
            &[
                ("E_anxiety", -0.30),
                ("E_fearfulness", -0.20),
                ("A_patience", 0.30),
        ...
        (
            ...
            &[
                ("X_liveliness", 0.40),
                ("X_sociability", 0.25),
                ("X_social_self_esteem", 0.20),
                ("O_creativity", 0.15),
            ],
        ),
        (
            ValueType::Craftsmanship,
            &[
                ("C_perfectionism", 0.35),
                ("C_diligence", 0.30),
                ("O_aesthetic", 0.20),
                ("C_organization", 0.15),
            ],
        ...
        (
            ValueType::Knowledge,
            &[
                ("O_inquisitiveness", 0.45),
                ("O_creativity", 0.25),
                ("C_diligence", 0.15),
                ("O_unconventionality", 0.15),
            ],
        ),
        (
            ValueType::Nature,
...
mod tests {
    ...
    fn all_33_value_types_have_seed_entries() {
        ...
        assert!(
            missing.is_empty(),
            "Missing ValueTypes in hexaco_seed_map: {:?}",
            missing
        );
        assert_eq!(
            map.len(),
            33,
            "Expected exactly 33 entries, got {}",
            map.len()
        );
    }
    ...
    fn all_24_facet_names_resolve_to_valid_indices() {
        ...
        for name in &all_facet_names {
            ...
            assert!(
                idx.expect("is_some checked on line above") < 24,
                "facet_index({:?}) out of range: {:?}",
                name,
                idx
            );
        }
    ...
    fn facet_index_all_24_canonical_names() {
        ...
        for (i, name) in names.iter().enumerate() {
            let idx = facet_index(name)
                .unwrap_or_else(|| panic!("facet_index({:?}) returned None", name));
            assert_eq!(
                idx, i,
                "facet_index({:?}) = {} but expected {}",
                name, idx, i
            );
            indices.insert(idx);
        }
