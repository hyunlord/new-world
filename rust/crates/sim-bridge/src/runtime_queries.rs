...
/// Computes band territory density textures for shader-based rendering.
/// Returns (band_id_map, density_map) — both sized width×height.
/// band_id_map: 0 = no band, 1..=N = band index (1-indexed).
/// density_map: 0–255 Gaussian density value.
pub(crate) fn compute_band_territory_textures(
    world: &World,
    resources: &sim_engine::SimResources,
) -> (Vec<u8>, Vec<u8>) {
    let (width, height) = resources.influence_grid.dimensions();
    let w = width as usize;
    let h = height as usize;
    let cell_count = w * h;

    let mut band_id_map = vec![0_u8; cell_count];
    let mut density_map = vec![0_u8; cell_count];

    // Build position map: entity raw_id → (x, y)
    let mut position_map: HashMap<u64, (f64, f64)> = HashMap::new();
    for (entity, pos) in world.query::<&Position>().iter() {
        position_map.insert(entity.id() as u64, (pos.x, pos.y));
    }

    let bands: Vec<&sim_core::band::Band> = resources.band_store.all().collect();

    for (band_idx, band) in bands.iter().enumerate() {
        let band_label = (band_idx as u8).wrapping_add(1);
        ...
        for member_id in &band.members {
            if let Some(&pos) = position_map.get(&member_id.0) {
                positions.push(pos);
            }
        }
        ...
        // Gaussian blob: radius=6 tiles, sigma=2.4
        let sigma: f64 = 2.4;
        let sigma_sq_2 = 2.0 * sigma * sigma;
        let search_radius: i32 = 9; // ~1.5 * 6
        ...
        for &(mx, my) in &positions {
            ...
            for dy in -search_radius..=search_radius {
                ...
                for dx in -search_radius..=search_radius {
                    ...
                    let value = (-dist_sq / sigma_sq_2).exp();
                    let quantized = (value * 255.0).min(255.0) as u8;
                    let pixel_idx = (ty as usize) * w + (tx as usize);

                    if quantized > density_map[pixel_idx] {
                        density_map[pixel_idx] = quantized;
                        band_id_map[pixel_idx] = band_label;
                    }
                }
            }
        }
