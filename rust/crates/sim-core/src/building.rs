...
/// A planned wall placement that an agent can pick up as a task (P2-B3).
///
/// Generated by `economy::generate_wall_ring_plans()` when a settlement needs
/// shelter. Builders claim plans during behavior selection and execute them via
/// `ActionType::PlaceWall`. On completion the wall is stamped onto `tile_grid`
/// and the plan is removed.
...
pub struct WallPlan {
    ...
    /// Entity that has claimed this plan, or None if unclaimed.
    pub claimed_by: Option<EntityId>,
    /// Tick when the plan was created — used for stale-plan cleanup.
    pub created_tick: u64,
}

/// A planned furniture placement (P2-B3).
///
/// Same lifecycle as `WallPlan` but executed via `ActionType::PlaceFurniture`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
