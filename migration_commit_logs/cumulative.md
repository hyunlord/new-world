...
## Commit 277
- `sim-systems/body`에 `job_satisfaction_score_batch`, `occupation_best_skill_index`, `occupation_should_switch`를 추가해 social 핫패스 반복 판단 루프를 Rust 함수로 확장.
- `sim-bridge`에 `body_job_satisfaction_score_batch`, `body_occupation_best_skill_index`, `body_occupation_should_switch` 메서드를 추가해 GDScript에서 batch/판정 경로를 직접 호출 가능하게 확장.
- `job_satisfaction_system.gd`에 profile runtime cache를 도입하고 job drift 후보 점수 계산을 Rust batch 호출로 일괄 처리(실패 시 fallback 유지).
...
## Commit 280
- `sim-systems/body`에 `reputation_event_delta`, `reputation_decay_value`를 추가해 reputation 도메인 핵심 수식 연산을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_reputation_event_delta`, `body_reputation_decay_value` 메서드를 추가해 GDScript에서 reputation 수식을 직접 호출 가능하게 확장.
...
## Commit 282
- `sim-systems/body`에 `stratification_gini`, `stratification_status_score`를 추가해 계층화 모니터 핵심 수식 연산을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_stratification_gini`, `body_stratification_status_score` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
...
## Commit 284
- `sim-systems/body`에 `stratification_wealth_score`를 추가해 stratification wealth score 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_stratification_wealth_score` 메서드를 추가해 GDScript에서 wealth score 수식을 직접 호출 가능하게 확장.
- `stratification_monitor.gd`의 `_compute_wealth_scores`를 Rust-first 호출 + fallback 구조로 전환.
- `settlement_culture.gd`의 plasticity 계산을 `body_value_plasticity` Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `13/56` 적용.

## Commit 285
- `sim-systems/body`에 `family_newborn_health`를 추가해 FamilySystem의 newborn health 핵심 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_family_newborn_health` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `family_system.gd`의 `_calc_newborn_health`를 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `14/56` 적용.

## Commit 286
- `sim-systems/body`에 `title_is_elder`, `title_skill_tier`를 추가해 TitleSystem의 핵심 판정 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_title_is_elder`, `body_title_skill_tier` 메서드를 추가해 GDScript에서 해당 판정 수식을 직접 호출 가능하게 확장.
...
## Commit 288
- `sim-systems/body`에 `tension_scarcity_pressure`, `tension_next_value`를 추가해 TensionSystem 핵심 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_tension_scarcity_pressure`, `body_tension_next_value` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `tension_system.gd`의 scarcity pressure/next tension 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `17/56` 적용.

## Commit 289
- `sim-systems/body`에 `resource_regen_next`를 추가해 ResourceRegenSystem의 핵심 리젠 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_resource_regen_next` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `resource_regen_system.gd`의 food/wood 리젠 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `18/56` 적용.

## Commit 290
- `sim-systems/body`에 `age_body_speed`, `age_body_strength`를 추가해 AgeSystem의 body 파생치 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_age_body_speed`, `body_age_body_strength` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `age_system.gd`의 speed/strength 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `19/56` 적용.

## Commit 291
- `sim-systems/body`에 `tech_discovery_prob`를 추가해 TechDiscoverySystem의 최종 확률 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_tech_discovery_prob` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `tech_discovery_system.gd`의 확률 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `20/56` 적용.

## Commit 292
- `sim-systems/body`에 `migration_food_scarce`, `migration_should_attempt`를 추가해 MigrationSystem 의사결정 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_migration_food_scarce`, `body_migration_should_attempt` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `migration_system.gd`의 식량 부족 판정 및 migration 시도 게이트를 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `21/56` 적용.

## Commit 293
- `sim-systems/body`에 `tech_cultural_memory_decay`를 추가해 TechMaintenanceSystem의 memory decay 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_tech_cultural_memory_decay` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `tech_maintenance_system.gd`의 cultural_memory 감소 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `22/56` 적용.

## Commit 294
- `sim-systems/body`에 `tech_modifier_stack_clamp`를 추가해 TechUtilizationSystem의 modifier stack clamp 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_tech_modifier_stack_clamp` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `tech_utilization_system.gd`의 stack clamp 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `23/56` 적용.

## Commit 295
- `sim-systems/body`에 `movement_should_skip_tick`을 추가해 MovementSystem의 연령별 skip 판정 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_movement_should_skip_tick` 메서드를 추가해 GDScript에서 해당 판정을 직접 호출 가능하게 확장.
...
## Commit 297
- `sim-systems/body`에 `building_campfire_social_boost`, `building_add_capped`를 추가해 BuildingEffectSystem 핵심 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_building_campfire_social_boost`, `body_building_add_capped` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `building_effect_system.gd`의 campfire day/night boost 선택 및 capped add 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `26/56` 적용.

## Commit 298
- `sim-systems/body`에 `tech_propagation_culture_modifier`, `tech_propagation_carrier_bonus`, `tech_propagation_final_prob`를 추가해 TechPropagation 핵심 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_tech_propagation_culture_modifier`, `body_tech_propagation_carrier_bonus`, `body_tech_propagation_final_prob` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
...
## Commit 300
- `sim-systems/body`에 `job_assignment_best_job_code`, `job_assignment_rebalance_codes`를 추가해 JobAssignment의 deficit/surplus 선택 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_job_assignment_best_job_code`, `body_job_assignment_rebalance_codes` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `job_assignment_system.gd`의 직업 선택/재균형 대상 선택 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `29/56` 적용.

## Commit 301
- `sim-systems/body`에 `stat_threshold_is_active`를 추가해 StatThresholdSystem의 조건 판정 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_stat_threshold_is_active` 메서드를 추가해 GDScript에서 해당 판정을 직접 호출 가능하게 확장.
...
## Commit 303
- `sim-systems/body`에 `personality_linear_target`를 추가해 PersonalityMaturation의 선형 타깃 계산 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_personality_linear_target` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `personality_maturation.gd`의 `_linear_target`를 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `32/56` 적용.

## Commit 304
- `sim-systems/body`에 `intelligence_effective_value`를 추가해 IntelligenceSystem의 유효값 계산 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_intelligence_effective_value` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `intelligence_system.gd`의 per-key effective 계산을 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `33/56` 적용.

## Commit 305
- `sim-systems/body`에 `intelligence_g_value`를 추가해 IntelligenceGenerator의 g 계산 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_intelligence_g_value` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
- `intelligence_generator.gd`의 `_generate_g`를 Rust-first 호출 + fallback 구조로 전환.
- `cd rust && cargo test -q`, `cd rust && cargo run -q -p sim-test` 통과 확인.
- Rust 전환 잔여량: 데이터 로더 축 `9/9` 완료, 시스템 실행 축(브리지 적용 기준) `34/56` 적용.

## Commit 306
- `sim-systems/body`에 `personality_child_axis_z`를 추가해 PersonalityGenerator의 자식 축 z-score 합성 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_personality_child_axis_z` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
...
## Commit 308
- `sim-systems/body`에 `stat_sync_derived_scores`를 추가해 StatSyncSystem 파생 스탯 계산 수식을 Rust 함수로 확장하고 단위 테스트를 추가.
- `sim-bridge`에 `body_stat_sync_derived_scores` 메서드를 추가해 GDScript에서 해당 수식을 직접 호출 가능하게 확장.
...
## Commit 315
- `intergenerational_system`의 핵심 수식(세대 부하 step, scar index 정규화, HPA 민감도, Meaney repair)을 Rust-first 경로로 이관.
- `sim-systems`에 intergenerational 순수 함수 4종과 단위 테스트 4건을 추가하고, `sim-bridge`에 대응 GDExtension 메서드를 노출.
- GDScript는 SimBridge 캐시 기반 Rust 호출 후 실패 시 기존 계산식 fallback을 유지해 기존 동작을 보존.
...
## Commit 322
- `coping_system`의 확률/선택 핵심 수식(learn probability, softmax pick)을 Rust-first 경로로 이관.
- `sim-systems`에 coping 순수 함수 2종과 단위 테스트 2건을 추가하고, `sim-bridge`에 대응 GDExtension 메서드를 노출.
- GDScript는 SimBridge 캐시 기반 Rust 호출 후 실패 시 기존 계산식 fallback을 유지해 기존 동작을 보존.

## Commit 323
- `emotion_system`의 mental break 핵심 수식(역치, 발동 확률, break type 결정)을 Rust-first 경로로 이관.
- `sim-systems`에 emotion break 순수 함수 3종과 단위 테스트 2건을 추가하고, `sim-bridge`에 대응 GDExtension 메서드를 노출.
- GDScript는 SimBridge 캐시 기반 Rust 호출 후 실패 시 기존 계산식 fallback을 유지해 기존 동작을 보존.
