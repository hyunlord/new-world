...
## Summary
`trait_violation_system`을 Rust runtime 지원 시스템으로 추가했다.  
위반 이력 누적/스트레스 주입/침투사고 처리의 full parity 이관 전 단계로, 이번 커밋에서는 trait 위반 관련 수식 경로를 Rust scheduler에서 baseline 실행하도록 포팅했다.

## Files Changed
- `rust/crates/sim-systems/src/runtime.rs`
  - `TraitViolationRuntimeSystem` 추가 (`SimSystem` 구현)
  - baseline 경로 실행:
    - `body::trait_violation_context_modifier(...)`
    - `body::trait_violation_facet_scale(...)`
    - `body::trait_violation_intrusive_chance(...)`
  - 현재 단계는 side-effect-free baseline (violation history/stress/memory mutation 없음)
  - 테스트 추가:
    - `trait_violation_runtime_system_baseline_runs_without_side_effects`
- `rust/crates/sim-bridge/src/lib.rs`
  - 런타임 지원 시스템 키에 `trait_violation_system` 추가
  - `runtime_supports_rust_system(...)` allowlist에 `trait_violation_system` 추가
  - `register_supported_rust_system(...)`에 `TraitViolationRuntimeSystem` 등록 분기 추가
  - 테스트 갱신:
    - `runtime_supports_expected_ported_systems`

## Tracking Data Updated
- `reports/rust-migration/data/runtime-registered-systems-v2.csv`
  - `trait_violation_system`의 `rust_runtime_impl=yes` 반영
...
- `cd rust && cargo test -p sim-systems runtime::tests::trait_violation_runtime_system_baseline_runs_without_side_effects -- --nocapture` : PASS
...
- Previous: `23 / 46 = 50.00%`
- Current: `24 / 46 = 52.17%`
- Remaining: `47.83%`

## Notes
- full parity 이관은 action 위반 이벤트 입력, history state 업데이트, stress/event 발행을 Rust 소유 상태로 이전해야 완료된다.
- 다음 후보는 `mortality_system` 또는 `behavior_system` baseline 포팅이다.
