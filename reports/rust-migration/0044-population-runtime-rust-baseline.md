...
## Summary
`population_system`을 Rust runtime 지원 시스템으로 추가했다.  
실제 출생/사망 처리(엔티티 생성/제거, 자원 소비, 이벤트 발행)의 full parity 이관 전 단계로, 이번 커밋에서는 인구 게이트 계산 경로를 Rust scheduler에서 baseline 실행하도록 포팅했다.

## Files Changed
- `rust/crates/sim-systems/src/runtime.rs`
  - `PopulationRuntimeSystem` 추가 (`SimSystem` 구현)
  - baseline 경로 실행:
    - `body::population_housing_cap(...)`
    - `body::population_birth_block_code(...)`
  - 현재 단계는 side-effect-free baseline (birth/death mutation 없음)
  - 테스트 추가:
    - `population_runtime_system_baseline_runs_without_side_effects`
- `rust/crates/sim-bridge/src/lib.rs`
  - 런타임 지원 시스템 키에 `population_system` 추가
  - `runtime_supports_rust_system(...)` allowlist에 `population_system` 추가
  - `register_supported_rust_system(...)`에 `PopulationRuntimeSystem` 등록 분기 추가
  - 테스트 갱신:
    - `runtime_supports_expected_ported_systems`

## Tracking Data Updated
- `reports/rust-migration/data/runtime-registered-systems-v2.csv`
  - `population_system`의 `rust_runtime_impl=yes` 반영
...
- `cd rust && cargo test -p sim-systems runtime::tests::population_runtime_system_baseline_runs_without_side_effects -- --nocapture` : PASS
...
- Previous: `20 / 46 = 43.48%`
- Current: `21 / 46 = 45.65%`
- Remaining: `54.35%`

## Notes
- full parity 이관은 출생 승인/자원 차감/스폰 좌표 탐색/정착지 멤버십 갱신/인구 이벤트 발행을 Rust 소유 상태로 이전해야 완료된다.
- 다음 후보는 `migration_system` 또는 `trait_violation_system` baseline 포팅이다.
