...
5. Run TARGETED tests + clippy during your turn (NOT the full workspace):
   ...
   - `cargo clippy -p <each crate you modified> -- -D warnings` (TARGETED, not workspace-wide). clippy is a compile-time lint with no per-binary launch tax, BUT `clippy --workspace` forces a FULL-workspace recompile — and that recompile is part of the compile-bound turn cost (PHASE 2c: the Generator-turn 1800s timeout was driven by full-workspace clippy recompiles, not the test-launch tax). The crates you edited are already compiled for your targeted `--test` runs, so `-p <crate>` clippy is marginal extra cost and gives fast lint feedback. The harness runs the authoritative full `cargo clippy --workspace -- -D warnings` in the post-turn gate — coverage is fully preserved.
   - ⚠️ **Do NOT run `cargo test --workspace` (or `cargo test -p sim-test` without `--test`), and do NOT run `cargo clippy --workspace`, during your turn.** The harness runs the authoritative full `cargo test --workspace` AND full `cargo clippy --workspace -- -D warnings` gates automatically AFTER your turn completes (and again in Step-0) — THOSE are the verdict authority, and their coverage is complete. Running the full workspace test yourself blows your time budget (each of ~260 test binaries pays a ~25s macOS first-launch assessment); running full-workspace clippy forces a full recompile (the compile-bound turn cost). Either can get your turn killed before you emit a result. Targeted `--test` + `-p <crate>` clippy in-turn, plus the harness's post-turn full gates = full coverage, on time.
