...
copy_runtime_libraries() {
    local output_dir="$1"
    rm -f "$BIN_DIR"/lib*.dylib "$BIN_DIR"/lib*.dylib.* "$BIN_DIR"/lib*.0.dylib "$BIN_DIR"/lib*.0.*.dylib 2>/dev/null || true
    if compgen -G "$output_dir"/lib*.dylib >/dev/null; then
        cp -R "$output_dir"/lib*.dylib "$BIN_DIR"/
    fi
    if compgen -G "$output_dir"/lib*.dylib.* >/dev/null; then
        cp -R "$output_dir"/lib*.dylib.* "$BIN_DIR"/
    fi
    if compgen -G "$output_dir"/lib*.0.dylib >/dev/null; then
        cp -R "$output_dir"/lib*.0.dylib "$BIN_DIR"/
    fi
    if compgen -G "$output_dir"/lib*.0.*.dylib >/dev/null; then
        cp -R "$output_dir"/lib*.0.*.dylib "$BIN_DIR"/
    fi
...
replace_rpath() {
    ...
    install_name_tool -rpath "$old_rpath" "$new_rpath" "$file_path" 2>/dev/null || {
        install_name_tool -delete_rpath "$old_rpath" "$file_path" 2>/dev/null || true
        install_name_tool -add_rpath "$new_rpath" "$file_path" 2>/dev/null || true
    }
}

trap cleanup EXIT

echo "=== WorldSim LLM Runtime Setup (macOS) ==="
echo "llama.cpp tag: $LLAMA_CPP_TAG"
echo "Model: $MODEL_FILENAME (~${MODEL_SIZE_MB} MB)"
echo

echo "[1/6] Checking prerequisites..."
for cmd in git cmake make curl xcode-select; do
...
if command -v clang++ >/dev/null 2>&1; then
    ...
    echo "ERROR: C++ compiler (clang++ or g++) is required."
    exit 1
fi
echo "  C++ compiler: $CXX"

if [ "$(uname -m)" = "arm64" ]; then
    echo "  CPU: Apple Silicon detected"
else
    echo "  CPU: Intel macOS detected"
fi
...
replace_rpath "$BIN_DIR/llama-server" "$OUTPUT_DIR" "@executable_path"
if [ -f "$BIN_DIR/llama-bench" ]; then
    replace_rpath "$BIN_DIR/llama-bench" "$OUTPUT_DIR" "@executable_path"
fi

find "$BIN_DIR" -maxdepth 1 \( -type f -o -type l \) -name 'lib*.dylib*' | while read -r dylib_path; do
    replace_rpath "$dylib_path" "$OUTPUT_DIR" "@loader_path"
done
